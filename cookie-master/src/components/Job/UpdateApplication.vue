<template>
    <div class="update-application-container">
      ...
      <div class="container mt-4" v-if="!loading && application">
        <div class="card">
          <div class="card-body">
            <h3 class="card-title">Update Application for {{ application.jobId }}</h3>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchApplication() {
        try {
          const response = await axios.get(`http://localhost:8088/applications/user/${this.currentUser.id}/job/${this.applicationId}`);
